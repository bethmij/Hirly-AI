...
const getJobApplication = async (jobId: string) => {
    try {
        const response = await axios.get(`http://localhost:4000/jobApplication?id=${jobId}`);
                ...
                {isLoading ? (
                    ...
                    <>
                        ...
                        {jobApplication.map((application) => (
                            <div key={application.userId} className="flex justify-between cursor-pointer w-full h-20 gap-x-10 mb-10 bg-blue rounded-md">
